	...
	setData(command, data) {
		...
		case 4:
			...
			if (data.length > 0) {
				rawData = data.map(ser => {
					...
					let window_id = ser.window_id.toString(16).toString("hex");

					let combined = serial + window_id;
		...
		case 5:
			...
			rawData = data.map((ser, index) => {
				if (index === 0) {
					let window_id = ser.window_id.toString(16);
					let crc = "99";
					...
					return window_id + crc + ser.ledState + result;
				} else {
					let window_id = ser.window_id.toString(16);
					...
					let combined = window_id + parsed + delay.toString("hex");
					return combined;
				}
			});
			this.data = rawData.join("");
