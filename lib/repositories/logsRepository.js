...
LogsRepository.prototype.set = function($happn, logObj) {
	...
	const { log } = $happn;
	const { logType, createdAt, typeId, serial } = logObj.meta;

	return new Promise((resolve, reject) => {
		data.set(
			`persist/logs/${logType}/${createdAt}/${typeId}/${serial}`,
			logObj,
			{},
			(err, response) => {
				if (err) {
...
LogsRepository.prototype.get = function($happn, path, from, to, limit) {
	const { log } = $happn;
	const { data } = $happn.exchange;

	const options = {
		fields: { data: 1, meta: 1 },
	...
	let criteria = {
		...(typeof from !== "function" &&
			...
			from !== undefined && {
				...
				"meta.logType": { $nin: ["DET_UPDATE", "DET_INSERT"] },
			}),
	};
