...
LogsRepository.prototype.set = function($happn, logObj) {
	...
	return new Promise((resolve, reject) => {
		console.log("SETTING LOG", logObj);
		data.set(
			`persist/logs/${logObj.logType}/${logObj.createdAt}/${logObj.typeId}/${logObj.serial}`,
			logObj,
			{},
			(err, response) => {
				if (err) {
...
LogsRepository.prototype.get = function($happn, path, from, to) {
	...
	const options = {
		fields: { createdAt: 1, logType: 1, events: 1, serial: 1, typeId: 1 },
		sort: { createdAt: 1 }
	};
	let criteria = {
		...(typeof from !== "function" &&
