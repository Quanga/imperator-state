...
NodeRepository.prototype.componentStart = function($happn) {
	const { nodeRepository, data, stateService } = $happn.exchange;
...
NodeRepository.prototype.componentStop = function($happn) {
...
NodeRepository.prototype.updateNodeData = function($happn, nodeObj) {
	...
	return getData()
		.then(res => {
			...
			let filteredResObjKeys = resObjKeys.filter(x => x !== "modifiedAt");
...
NodeRepository.prototype.cutPath = function($happn, parentPath) {
	const { data } = $happn.exchange;
	console.log("PATH TO CUT", parentPath);
	return new Promise((resolve, reject) => {
