...
NodeRepository.prototype.get = function($happn, path, asUnits) {
	...
	return new Promise(resolve => {
		data.get(`/nodes/${path}`, null, (err, response) => {
			if (err) {
	...
	return (async () => {
		try {
			...
			await new Promise((resolve, reject) => {
				data.set(`/nodes/${path}`, nodeObj, {}, (error, response) => {
...
NodeRepository.prototype.cutPath = function($happn, parentPath) {
	...
	return new Promise((resolve, reject) => {
		data.remove(`/nodes/${parentPath}/*`, null, (error, result) => {
			if (error) return reject(error);
...
NodeRepository.prototype.delete = function($happn, path) {
	...
	return new Promise((resolve, reject) => {
		data.remove(`/nodes/${path}`, null, (error, result) => {
			if (error) return reject(error);
