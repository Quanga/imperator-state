...
DataMapper.prototype.mapUpdateNode = function(parsedPacket, foundResult) {
	let mapAsync = async () => {
		try {
			...
			if (changes.length > 0) {
				foundResult.meta.dirty = changes;
				let timestamp = new Date(parsedPacket.meta.storedPacketDate)
					.toISOString()
					.slice(0, 19);
				//.replace("T", " ");

				foundResult.data.modified = timestamp;
			}
			...
			// 		null,
			// 		2
			// 	)} with ${JSON.stringify(foundResult, null, 2)}`
			// );

			foundResult = await mapProps(parsedPacket, foundResult);
			console.log("changed", foundResult);
		} catch (err) {
