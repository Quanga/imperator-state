const { DefaultConstants } = require("../configs/uiConstants/defaultAppConstants");
const fs = require("fs");
const path = require("path");
const os = require("os");

/**
 * @module lib/services/SystemServices
 */
/**
 * @class
 */
class SystemService {
     ...
     * @mermaid graph LR
                    ...
                    H-->return
     */
	async checkConfiguration($happn) {
		...
		try {
			...
			if (!config) {
				...
				config = await systemService.setConfigData(DefaultConstants(), "persist/configuration");
	 ...
	 */
	async setConfigData($happn, payload) {
	 ...
	 */
	async upsertHistory($happn, historyObj) {
		const { systemRepository } = $happn.exchange;
		const { log } = $happn;
		...
		try {
			...
			propKeys.forEach((prp) => {
				if (Object.hasOwnProperty.call(history, prp)) {
	...
	async setWifiSettings($happn, settingsObj, filename) {
	...
	writeJSON($happn, filename, jsonObj) {
		const { log } = $happn;
