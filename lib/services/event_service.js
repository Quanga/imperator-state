...
EventService.prototype.componentStart = function($happn) {
	...
	return (async () => {
		...
		this.emitQueue = new Queue((task, cb) => {
			console.log("EVENT EMIT", task.path);
...
EventService.prototype.handleEvent = function($happn, eventObj) {
	...
	const { log } = $happn;
	//console.log("event", JSON.stringify(eventObj, null, 2));

	return (async () => {
		try {
			const { logType } = eventObj.meta;
			...
			switch (logType) {
				...
				case "dataService/UNITS_INSERTED":
					{
						let path = "eventService/UNIT_UPDATE";

						this.emitQueue.push({ path, eventObj });
						await logsRepository.set(eventObj);
