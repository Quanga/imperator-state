...
EventService.prototype.handleEvent = function($happn, eventObj) {
	...
	return (async () => {
		try {
			const { payload, typeId, created, serial } = eventObj;

			let eventLog = new EventLog();
			eventLog.setId({ serial, typeId, created });
			...
			await logsRepository.insertLog(eventLog);
			await eventService.processWarnings();
