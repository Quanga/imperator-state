			...
			case dataServiceEvents.UNITS_UPDATED:
				{
					...
					for (const rKey of resultKeys) {
						...
						await logsRepository.set(JSON.stringify(eventLog));
						await eventService.processWarnings(eventLog);
					}
				}
				break;
...
EventService.prototype.acknowledgeWarning = function($happn, id, username, createdAt) {
	...
	return (async () => {
		let record = await warningsRepository.getById(id);
		record.ack = true;
		record.ackBy = username;
		record.ackDate = createdAt;
		record.ackType = "user";
		delete record._meta;

		let response = await warningsRepository.set(record);
		return response;
	})();
};
