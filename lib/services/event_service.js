...
EventService.prototype.handleEvent = function($happn, eventObj) {
	...
	return (async () => {
		try {
			...
			const reducedLogs = Object.prototype.hasOwnProperty.call(eventObj, "payload")
				? this.reduceLogs(payload)
				: null;
							...
							switch (rKey) {
								...
								default:
									...
									if (result[rKey].length > 0) {
										const diffs = result[rKey].filter(u => Object.prototype.hasOwnProperty.call(u, "diff"));
										if (diffs.length > 0) {
											eventLog.setEvents(
												diffs.map(d => {
													return { diff: d.diff };
...
EventService.prototype.reduceLogs = function(payload) {
	let result = payload.reduce((acc, cur) => {
		if (!Object.prototype.hasOwnProperty.call(acc, cur.data.typeId)) {
			acc[cur.data.typeId] = [];
		}
