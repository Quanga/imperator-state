...
EventService.prototype.initialise = function($happn) {
	...
	const initAsync = async () => {
		try {
			...
			let processedNodes = await $happn.exchange.dataService.buildTree();
			this.dataModel = processedNodes;
			// console.log($happn.exchange);
			//****************************************************************** */
			...
			if (controlUnit) {
				this.blastModel.initialise(controlUnit);
			} else {
			...
			if (config.systemType === "AXXIS") {
				...
				for (const unit of this.dataModel) {
					if (unit.data.key_switch_status === 1 && unit.data.type_id === 3) {
						this.blastModel.addToBlast(unit, { status: "active" });
					}
					// if (unit.keyswitch_status === 1) {
					// 	this.blastModel.addToBlast(unit, { status: "active" });
					// }
				}
			}

			//console.log(this.blastModel);
			...
			if (config.systemType === "IBS") {
				...
				for (const unit of this.dataModel) {
					if (unit.data.key_switch_status === 1 && unit.data.type_id === 2) {
						this.blastModel.addToBlast(unit, { status: "active" });
					}
					// if (unit.keyswitch_status === 1) {
					// 	this.blastModel.addToBlast(unit, { status: "active" });
			...
			this.dataModel.forEach(element => {
				...
				element.on("started", function(name) {
					//console.log("hearing emit", name);
				});
			});
	...
	let Process = async () => {
		...
		for (const updatedNode of context.updateNodes) {
			if (updatedNode.meta.dirty && updatedNode.meta.dirty.length > 0) {
				...
				let deconDirty = updatedNode.meta.dirty.reduce((accum, [k, v]) => {
					accum[k] = v["incoming"];
					return accum;
				}, {});

				deconDirty.serial = updatedNode.data.serial;
				deconDirty.type_id = updatedNode.data.type_id;
