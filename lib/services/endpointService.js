	...
	async checkForEndpoint($happn) {
		...
		try {
			...
			this.check = setInterval(() => {
				log.info(`Checking for Endpoint ${process.env.INPUT_MODULE_NAME} -\
	...
	async initialise($happn) {
		...
		try {
			...
			if (activeQueues.findIndex((queue) => queue === process.env.INPUT_MODULE_USER) === -1) {
				...
				log.info(`Creating queue for ${process.env.INPUT_MODULE_USER} \
