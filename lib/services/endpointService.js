	...
	return (async () => {
		try {
			...
			this.check = setInterval(() => {
				log.info(`Checking for Endpoint ${env.endpointName} - ${env.endpointIP}:${env.endpointPort}`);
	...
	return (async () => {
		try {
			...
			if (activeQueues.findIndex(queue => queue === env.endpointUsername) === -1) {
				...
				log.info(`Creating queue for ${env.endpointUsername} with last know packet ${lastPacket || 0}`);
