...
EndpointService.prototype.componentStop = function($happn) {
	...
	return new Promise(resolve => {
		...
		if (this.client && this.status === "CONNECTED") {
			const { queueService: evQueueService } = this.client.event[process.env.INPUT_INSTANCE_NAME];
	...
	return (async () => {
		try {
			...
			this.check = setInterval(() => {
				log.info(
					`Checking for Endpoint ${process.env.INPUT_INSTANCE_NAME} -\
					 ${process.env.INPUT_INSTANCE_HOST}:${process.env.INPUT_INSTANCE_PORT}`,
				);

				tcpPortUsed.check(process.env.INPUT_INSTANCE_PORT, process.env.INPUT_INSTANCE_HOST).then(
					function(inUse) {
				...
				tcpPortUsed
					.waitUntilUsedOnHost(
						process.env.INPUT_INSTANCE_PORT,
						process.env.INPUT_INSTANCE_HOST,
						500,
						2000,
					)
					.then(
						() => {
							clearInterval(this.check);
							clearTimeout(this.checkTimer);
							log.info(`Connecting to Endpoint ${process.env.INPUT_INSTANCE_NAME}`);
							endpointService.connectToEndpoint();
						},
						err => {
							log.warn(
								`${process.env.INPUT_INSTANCE_HOST}:${process.env.INPUT_INSTANCE_PORT} not available`,
							);
						},
					);
			}, process.env.EP_CHECK_INTERVAL);
...
EndpointService.prototype.connectToEndpoint = function($happn) {
	...
	this.client = new Mesh.MeshClient({
		secure: true,
		host: process.env.INPUT_INSTANCE_HOST,
		port: process.env.INPUT_INSTANCE_PORT,
	});

	return new Promise((resolve, reject) => {
		this.client.on("login/allow", () => {
			log.info(
				`Connected to Endpoint ${process.env.INPUT_INSTANCE_NAME} as ${process.env.INPUT_INSTANCE_USER}`,
			);
		...
		this.client.on("reconnect/successful", () => {
			...
			if (this.client.data.status === 0) {
				this.client.login({
					username: process.env.INPUT_INSTANCE_USER,
					password: process.env.INPUT_INSTANCE_PASSWORD,
				});
			} else {
				endpointService.initialise();
			}
...
EndpointService.prototype.initialise = function($happn) {
	...
	return (async () => {
		try {
			const { queueService: evQueueService } = this.client.event[process.env.INPUT_INSTANCE_NAME];
			const { queueService: epQueueService } = this.client.exchange[process.env.INPUT_INSTANCE_NAME];
			...
			if (activeQueues.findIndex(queue => queue === process.env.INPUT_INSTANCE_USER) === -1) {
				...
				log.info(
					`Creating queue for ${process.env.INPUT_INSTANCE_USER} with last know packet ${lastPacket ||
						0}`,
				);

				await epQueueService.buildQueue(process.env.INPUT_INSTANCE_USER, lastPacket || 0);
			}

			log.info(
				`Connecting to queue - ${process.env.INPUT_INSTANCE_NAME} - ${process.env.INPUT_INSTANCE_USER}`,
			);

			const queueCount = await epQueueService.size(process.env.INPUT_INSTANCE_USER);
			log.info(`Downloading ${queueCount} items from queue`);

			await endpointService.checkQueue();

			log.info(
				`Listening to queue - ${process.env.INPUT_INSTANCE_NAME}-${process.env.INPUT_INSTANCE_USER}`,
			);

			if (this.client.data.status === 1 && !this._epRef) {
				evQueueService.on(
					`enqueue/${process.env.INPUT_INSTANCE_USER}`,
					data => {
	...
	return (async () => {
		try {
			if (this.status === "CONNECTED") {
				let queueCount = await epQueueService.size(process.env.INPUT_INSTANCE_USER);
				if (queueCount === 0) {
				...
				for (let i = 0; i < queueCount; i++) {
					let packet = await epQueueService.dequeue(process.env.INPUT_INSTANCE_USER);

					if (packet) {
						this.incomingQueue.push(packet);
					} else {
						log.error("packet", packet);
						queueCount = await epQueueService.size(process.env.INPUT_INSTANCE_USER);
