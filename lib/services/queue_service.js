...
QueueService.prototype.processIncoming = function($happn, msgObj) {
	const { parserService, dataService, eventService, queueService } = $happn.exchange;
	const { log } = $happn;
	...
	return (async () => {
		try {
			...
			const unitsObj = await parserService.extractData(msgObj); //returns array of unit objects
			if (
				!unitsObj ||
