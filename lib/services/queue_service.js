	...
	return (async () => {
		try {
			...
			if (!unitsArr || !Array.isArray(unitsArr) || unitsArr.length === 0)
				throw new Error(
					`Packet could not be processed - no data will be sent to DataService`,
				);
...
QueueService.prototype.validatePacket = function($happn, msgObj) {
	return (async () => {
		...
		if (process.env.NODE_ENV === "test") {
			...
			const packetCRC = msgObj.packet.substring(
				msgObj.packet.length - 4,
				msgObj.packet.length,
			);

			if (crcCalc !== packetCRC)
				throw new Error(`Packet ${msgObj} has and invalid CRC format`);
		}
	})();
};

QueueService.prototype.addToQueue = function($happn, $origin, opts, callback) {
	...
	return (async () => {
		...
		await queueService.processIncoming(messageObj);
