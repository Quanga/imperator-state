...
const { blastStates, componentStates } = require("../constants/stateConstants");
const {
	blastModelEvents,
	blastServiceEvents,
	eventServiceEvents
	...
	return (async () => {
		try {
			if (this.currentBlast) {
				...
				await blastService.logEvent("BLAST_ERROR_LOG", msg);
				log.warn(msg);
				return;
			}
			...
			await blastService.logEvent("BLAST_STARTED", this.currentBlast.blastEvent.data.createdAt);
		} catch (err) {
...
BlastService.prototype.subscribeToBlastEvents = function($happn) {
	...
	return (async () => {
		try {
			blastEvent.event.on(blastModelEvents.BLASTMODEL_LOG, data => {
				const { BLAST_TIMER_COMPLETE } = blastStates;
				if (data === BLAST_TIMER_COMPLETE || data === BLAST_TIMER_COMPLETE) {
...
BlastService.prototype.closeBlast = function($happn) {
	...
	return (async () => {
		try {
			if (!this.currentBlast) throw new Error("No blast event to close");

			this.currentBlast.blastEvent.event.removeAllListeners("BLASTMODEL_LOG");
			...
			await blastService.logEvent("BLAST_COMPLETED", this.currentBlast.blastEvent.data.blastClosed);
			this.currentBlast = null;
		} catch (err) {
 ...
 * @returns {Promise} void
 */
BlastService.prototype.logEvent = function($happn, logType, createdAt) {
...
BlastService.prototype.startTimer = function($happn, timer, event, duration, callback) {
	...
	this[timer].on("tick", ms => emit(event, ms));
	this[timer].on("done", () => {
