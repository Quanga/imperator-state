	...
	return (async () => {
		try {
			dataService.on(
				"state",
				(data, meta) => {
					//console.log("HEARING STATE", data);
					if (data[typeId] === 0) {
	...
	return (async () => {
		try {
			...
			this.currentBlast = BlastModel.create(timeStamp)
				.withOpts({ firingDuration, reportingDuration })
				.withSnapshot(snapShot)
				.withFSM(fsm)
				.on("log", msg => log.info(msg))
				.on("state", state => blastService.logEvent("blastModel/STATE", state))
...
BlastService.prototype.closeBlast = function($happn, closeProc) {
	...
	return (async () => {
		try {
			...
			this.currentBlast.event.removeAllListeners("diff");
