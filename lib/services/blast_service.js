	...
	return (async () => {
		try {
			...
			dataService.on(
				"BLAST_STARTED",
				data => {
					blastService.createNewBlast(data.createdAt, data.snapShot);
				},
				(err, eventRef) => {
...
BlastService.prototype.createNewBlast = function($happn, createdAt, snapShot) {
	...
	return (async () => {
		try {
			...
			this.currentBlast = {
				createdAt: createdAt,
				blastEvent: new BlastModel(snapShot, createdAt, env.systemReportTime + env.systemFiringTime)
			};
...
BlastService.prototype.reportTimer = function($happn, start) {
	...
	if (start) {
		log.info(`Blast Report Started at ${countdown}ms`);
		...
		this._reportTimer = setInterval(() => {
			if (countdown > 1) {
				...
				log.info(`Blast Report Timer Stopped`);
				emit("REPORT_TIMER", 0);
