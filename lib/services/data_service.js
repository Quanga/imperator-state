...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr, command) {
	...
	const processUpsert = async () => {
		try {
			...
			await dataService.postProcess(context.nodeArr);
		} catch (err) {
...
DataService.prototype.postProcess = function($happn, currentState) {
	const { blastService } = $happn.exchange;
	const parent = currentState[0].data;
	const { meta } = currentState[0];

	const postProcessAsync = async () => {
		switch (parent.typeId) {
		case 0:
			{
				if (parent.keySwitchStatus === 1 && parent.fireButton === 1) {
					console.log("LLLLLLLLLLL>>>>>>>>>>>>>>>>", parent);
					return blastService.createNewBlast(meta.storedPacketDate);
				}
			}
			break;
		default: {
