	...
	return (async () => {
		try {
			...
			case "EDD_CLEARSIG":
				...
				updateRes = await dataService.updateState([nodeArr[0]], false);
		...
		try {
			...
			if (resultObj.inserts.length > 0) {
				...
				await eventService.handleEvent({
					...
					payload: clone(resultObj.inserts)
				});
				console.log("UPDATES", JSON.stringify(resultObj.inserts, null, 2));
			}
			...
			if (resultObj.updates.length > 0) {
				...
				// if (
				// 	resultObj.updates[0].data.typeId === 3 &&
				// 	parseInt(resultObj.updates[0].data.serial, 10) === 77
				// ) {
				// 	console.log(resultObj.updates[0]);
				// }
				// if (!arr[0].data.modifiedAt) {
				// 	console.log("NO MODIFIED ------", resultObj.updates);
				// }
				...
				await eventService.handleEvent({
					...
					typeId: arr[0].data.typeId,
					payload: clone(resultObj.updates)
				});
