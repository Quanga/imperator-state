...
DataService.prototype.__buildTree = function($happn) {
	...
	const buildTreeAsync = async () => {
		try {
			...
			//console.log("$happn", JSON.stringify($happn.data, null, 2));
	...
	let insNode = async () => {
		...
		if (node.serial != null) {
			try {
				...
				nodeClone.id = nodeId;
				await eventService.addNode(nodeClone);
...
DataService.prototype.__updateNode = function($happn, node) {
	...
	let updateNodeAS = async () => {
		try {
			let nodeClone = JSON.parse(JSON.stringify(node));
			await eventService.updateNode(nodeClone);
...
DataService.prototype.__setParentId = function(currentNode, subTree) {
	let parentId = null;
	let parent = this.__nodeTreeUtil.findParent(subTree, currentNode.type_id);
