...
DataService.prototype.validatePersisted = function($happn, units) {
	...
	return (async () => {
		try {
			...
			let controlUnit = await units.filter(u => u.data.typeId === 0);

			if (controlUnit && controlUnit.length > 0) {
				if (controlUnit.length > 1) {
					const allCu = controlUnit.reduce((acc, cur) => acc.concat(`-${cur.data.path}  `), "");
					controlUnit = controlUnit.sort((a, b) => a.data.modifiedAt < b.data.modifiedAt);
					log.warn(`Too Many control units found.... please remove one of ${allCu}`);
					log.warn(`Adding latest to Blast Model - unit ${controlUnit[0].data.path}`);
				}

				resultArr.push(controlUnit[0]);
				log.info(`Adding Control Unit ${controlUnit[0].data.serial}`);
			} else {
				log.warn("No Control Unit found...");
			}

			let boosters = units.filter(u => u.data.typeId === 3);

			if (boosters && boosters.length > 0) {
				log.info(`Adding ${boosters.length} boosters to the Data Model`);
				for (let booster of boosters) {
					log.info(`Adding ${booster.data.serial} to the Data Model`);
			...
			let dets = units.filter(u => u.data.typeId === 4);
	...
	return (async () => {
		try {
			...
			switch (preProcess.type) {
				...
				case "EDD_CLEARSIG":
					{
						const sigUnit = clone(nodeArr[0]);
						await this.dataModel.resetChildCount(sigUnit);
						sigUnit.data.childCount = 0;
						updateRes = await dataService.updateState([sigUnit]);

						if (!sigUnit.data.path) sigUnit.setPath();

						await eventService.handleEvent({
							type: dataServiceEvents.EDD_SIGNAL_DETECTED,
							serial: sigUnit.data.serial,
							createdAt: sigUnit.data.createdAt,
							typeId: sigUnit.data.typeId,
						});
						await nodeRepository.cutPath(sigUnit.data.path);
					}
					break;
		...
		try {
			...
			if (parentType === 3) {
				...
				if (nodeArr[0].dataType === "data") {
					...
					if (result.length === 2 && result[1].data.delay === 255 && result[1].data.windowId === 255) {
	...
	return (async () => {
		try {
			for (let update of updates) {
				if (update && Object.prototype.hasOwnProperty.call(update, "data")) {
					await nodeRepository.insertNodeData({ data: { ...update.data }, meta: { ...update.meta } });
				}
			}
		} catch (err) {
