...
class DataService {
	...
	async initialise($happn) {
		...
		try {
			...
			if (persisted && persisted.length > 0) {
				await dataService.rehydrateData(persisted);
				return true;
			}
	 ...
	 * @returns {Promise}
	 */
	async createDataModel($happn) {
		const { dataService } = $happn.exchange;
		const { log, emit } = $happn;
		const model = DataModel.create().withMode(process.env.MODE);
		model
			.on("error", (err) => log.error("DataModel error:", err))
			.on("state", (state) => emit("state", state))
	 ...
	 * @returns {Promise}
	 */
	async rehydrateData($happn, persisted) {
		const { dataService } = $happn.exchange;
		const { log } = $happn;

		try {
			await dataService.upsertNodeDataArr(persisted, false);
		} catch (error) {
			log.error(error);
	 ...
	 * @returns {Promise}
	 */
	async processUnitObj($happn, unitsObj) {
		const { dataService } = $happn.exchange;
		const { log } = $happn;
	 ...
	 * @returns {Promise}
	 */
	async upsertNodeDataArr($happn, nodeArr, withEmit) {
		const { dataService } = $happn.exchange;
		const { log } = $happn;
		...
		try {
			...
			const updatedContext = await dataService.commitUpdates(context);
			if (typeof withEmit === "function") {
				await Promise.all([
					dataService.reportUpserts(updatedContext),
					dataService.persistUnits([...updatedContext.commits.INSERT, ...updatedContext.commits.UPDATE]),
				]);
			}
     ...
     */
	async createContext($happn, nodeArr) {
		const { log } = $happn;
     ...
     */
	async commitUpdates($happn, context) {
		const { log } = $happn;
		const { upserts } = context;

		try {
			context.commits = Object.keys(upserts).reduce((acc, cur) => {
				acc[cur] = Object.keys(upserts[cur]).map((unit) =>
					this.dataModel.commitUpsert({ ...upserts[cur][unit] }),
				);
				return acc;
			}, {});
			return context;
	...
	// DataService.prototype.extractUpserts= function ($happn, upserts, upsertKey, dataService, context) {
	//
	// }
	...
	async handleSignal($happn, unitsObj) {
	...
	async reportUpserts($happn, context) {
		const { log, emit } = $happn;
		const { serial, createdAt, typeId, windowId } = fields;
		const { upserts } = context;
		const generateMessage = (item) => {
			let message = item.value.nextState.meta[windowId]
				? `[ type:${item.value.nextState.meta[typeId]}, windowId: ${item.value.nextState.meta[windowId]} ]`
				: `[ type:${item.value.nextState.meta[typeId]}, serial: ${item.value.nextState.meta[serial]} ]`;
			if (Object.keys(item.value.diffs).length > 0)
				message = message.concat(` - diffs: ${JSON.stringify(item.value.diffs)}`);
			log.info(message);
		};

		try {
			Object.keys(context.commits).forEach((u) => {
				if (upserts[u].length > 0) {
					log.info(`[ UNITS ${u} ] - ${upserts[u].length} unit${upserts[u].length > 1 ? "s" : ""}`);
					upserts[u].forEach((item) => generateMessage(item));
					const eventLog = EventLog.create(context.nodeArr[0].meta[createdAt])
						.setLogType(`dataService/UNITS_${u}`)
						.setSerial(context.nodeArr[0].meta[serial])
						.setTypeId(context.nodeArr[0].meta[typeId])
						.withMessage(upserts[u])
						.setEvents();
					emit("log", eventLog);
				}
			});
	 ...
	 */
	async persistUnits($happn, updates) {
		const { nodeRepository } = $happn.exchange;
		const { log } = $happn;
	 ...
	 */
	async getUnits($happn, withChildren) {
		const { log } = $happn;
	...
	async getSnapShot($happn) {
		await this.dataModel.snapShot();
	}

	async getDataModel($happn) {
		const { log } = $happn;
	...
	async updateUnitMeta($happn, nextState) {
		const { log } = $happn;
		const { nodeRepository } = $happn.exchange;
