	...
	return (async () => {
		try {
			...
			case "EDD_CLEARSIG":
				...
				//updateRes = await dataService.updateState(nodeArr[0], true);
				await nodeRepository.cutPath(nodeArr[0].data.path);
				await eventService.handleEvent({ type: "EDD_SIG", payload: nodeArr[0] });
				break;
	...
	return (async () => {
		try {
			...
			if (resultObj.updates.length > 0) {
				...
				await dataService.updateExistingNodes(resultObj.updates);
				await eventService.handleEvent({ type: "UPDATE", payload: resultObj.updates });
			}

			if (resultObj.inserts.length > 0) {
				log.info("DATA UPDATE - inserting new units", resultObj.inserts.length);
				await dataService.insertNewNodes(resultObj.inserts);
				await eventService.handleEvent({ type: "INSERT", payload: resultObj.inserts });
			}
					...
					if (
						...
						if (boosters.length > 0) {
							for (let booster of boosters) {
								...
								await dataService.updateExistingNodes(boosterArr);
							}
						}
					}
				}
			...
			case 3:
				{
					if (
						...
						// if (dets.length > 0) {
						// 	await dataService.updateExistingNodes(dets);
						// }
					}
				}
 ...
 */
DataService.prototype.insertNewNodes = function($happn, inserts) {
	...
	return (async () => {
		try {
			for (let insert of inserts) {
				const nodeClone = { ...insert.data };
				await nodeRepository.insertNodeData(nodeClone);
			}
		} catch (err) {
			log.error("Insert Node Error:", err);
			return Promise.reject(err);
		}
	})();
};
 ...
 */
DataService.prototype.updateExistingNodes = function($happn, updates) {
	const { nodeRepository } = $happn.exchange;
	const { log } = $happn;
