...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr, command) {
	...
	const updateState = async checkedArr => {
		let updates = [];
		let inserts = [];

		for (const unit of checkedArr) {
			let result = await this.dataModel.upsertUnit(unit);
	...
	const processUpsert = async () => {
		try {
			...
			if (checkClear.action === "EDD_CLEARSIG") {
				let result = await this.dataModel.clearEdds(nodeArr[0]);
				await dataService.clearEddSignal(nodeArr);
			}
...
DataService.prototype.preProcess = function($happn, arr) {
	...
	let preProcessAsync = async () => {
		if (parentType === 3) {
			...
			if (arr[0].dataType === "data" && arr.length === 2) {
...
DataService.prototype.getUnits = function($happn, withChildren = false) {
	const getAsync = async () => {
		let units = this.dataModel.units;

		let result = [];
		if (!withChildren) {
			result = units.map(unit => {
				const { children, ...noChildren } = unit;
				return noChildren;
			});
		} else {
			result = [...units];
