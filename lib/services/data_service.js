...
DataService.prototype.rehydrateData = function($happn, model, persisted) {
	...
	return (async () => {
		try {
			...
			// }
			//const upserts = Promise.all(persisted.map(unit=> model.stageUpsert(unit, true)));
...
DataService.prototype.getPersistedUnits = function($happn) {
	...
	return (async () => {
		try {
			...
			console.log("persisted units", unitsObj);
			let resultArr = [];
...
DataService.prototype.getControlUnit = function($happn) {
	...
	return (async () => {
		try {
			let result = await this.dataModel.snapShot();
...
DataService.prototype.updateUnitMeta = function($happn, nextState) {
	...
	return (async () => {
		try {
			const result = await this.dataModel.updateUnitMeta(nextState);
			const update = await nodeRepository.insertNodeData({ ...result });
			console.log("resturn from the update", update);
