...
DataService.prototype.initialise = function($happn) {
	let initAsync = async () => {
		//create in-memory tree of nodes
		let nodeFullTree = await this.__buildTree($happn);
		await this.setData($happn, "nodeTree/", nodeFullTree);

		//for test
		let fulltreeget = await this.getData($happn, "nodeTree/*");
		console.log(fulltreeget);
	};

	return initAsync();
};

DataService.prototype.setData = function($happn, path, data) {
	const { data: dataLayer } = $happn;

	return new Promise((resolve, reject) => {
		dataLayer.set(path, { data: data }, { noPublish: true }, (err, result) => {
			if (err) {
				reject(err);
			}
...
DataService.prototype.getData = function($happn, path) {
	const { data: dataLayer } = $happn;

	return new Promise((resolve, reject) => {
		dataLayer.get(path, null, (err, result) => {
			if (err) {
				reject(err);
			}
...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr) {
	...
	const buildTreeAsync = async () => {
		try {
			//get all
			//for test
			//let fullTreeFromDL = await this.getData($happn, "nodeTree/*");
			//console.log(fulltreeget);
			fullTree = await this.__buildTree($happn);
			//fullTree = fullTreeFromDL[0].data;
