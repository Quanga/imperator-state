...
DataService.prototype.__insertNode = function($happn, node) {
	const { nodeRepository, serverService } = $happn.exchange;
	...
	let insNode = async () => {
		...
		if (node.serial != null) {
			try {
				...
				//$happn.exchange.severService.apiModel(nodeClone);
				const apiExchange = await serverService.serverApi();
				console.log("API EXCHANGE ", apiExchange);
				apiExchange.exchange.nodeRepository
					.insertNodeData(nodeClone)
					.then(id => {
