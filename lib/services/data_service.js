	...
	return (async () => {
		try {
			...
			if (validatedUnits) {
				...
				log.warn("No Units found in Node Repository.... Blast Model is empty");
			}
		} catch (err) {
...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr) {
	...
	return (async () => {
		try {
			...
			case "EDD_CLEARSIG":
				{
					...
					let updateRes = await dataService.updateState(clearRes, true);
			...
			case "FILTERED":
				{
					let updateRes = await dataService.updateState(checkClear.value);
			...
			case "CLEAR":
				{
					let updateRes = await dataService.updateState(nodeArr);
			...
			case "COMM_LOST":
				{
					let updateRes = await dataService.updateState(nodeArr, false);
...
DataService.prototype.updateState = function($happn, nodeArr, force) {
	...
	return (async () => {
		try {
			let res = { updates: [], inserts: [] };

			for (const unit of nodeArr) {
				...
				case "INSERT":
					res.inserts.push(upsertResult);
					break;
				case "UPDATE":
					res.updates.push(upsertResult);
					break;
				case "NONE":
					break;
				default:
			...
			if (res.updates.length > 0) {
				log.info("DATA UPDATE - updating existing units", res.updates.length);
				await this.__updateExistingNodes($happn, res.updates);
			}

			if (res.inserts.length > 0) {
				log.info("DATA UPDATE - inserting new units", res.inserts.length);
				await this.__insertNewNodes($happn, res.inserts);
			}
