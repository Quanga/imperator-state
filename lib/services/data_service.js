...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr) {
	...
	let updateChildLogs = async () => {
		...
		try {
			if (context.updateNodes.length > 0) {
				context.updateNodes.forEach(async updatedNode => {
				...
				console.log("test", context.logs);

				context.logs.forEach(async createdLog => {
					console.log("running all logs out");

					console.log("test", createdLog);
					await logsRepository.insertLogData(createdLog);
				});
			}
		} catch (err) {
	...
	let insNode = async () => {
		...
		if (node.serial != null) {
			try {
				...
				let nodeId = await repository.insertNodeData(nodeClone);
				return nodeId;
			} catch (err) {
	...
	let updateExistingAsync = async () => {
		for (const node of context.updateNodes) {
			try {
				...
				logError(
					`error updating node ${JSON.stringify(node, null, 2)} -  ${err} `
				);
			}
		}
	};
...
DataService.prototype.__createLogs = function($happn, updatedNode) {
	...
	let createLogsAsync = async () => {
		...
		let constantData;
		let { incomingCommands } = this.__constants;

		switch (updatedNode.type_id) {
		case 0:
			constantData = incomingCommands[8].data.first_byte.bits;
			break;
		case 2:
			constantData = constantData =
					incomingCommands[3].commandConstant.data.remaining_bytes.bits;
			break;
		default:
			constantData = incomingCommands[3].commandConstant.data.first_byte.bits;
			break;
		}

		if (updatedNode.type_id < 4) {
			updatedNode.dirty.forEach(change => {
				let logItem = this.createLogResultObject();
				try {
					let incomingVal = change[1]["incoming"];
					let constData = constantData[change[0]];

					logItem.node_serial = updatedNode.serial;
					logItem.node_type_id = updatedNode.type_id;

					logItem.message = `${constData.memo} has been ${
						incomingVal == 1 ? constData.true : constData.false
					}`;
