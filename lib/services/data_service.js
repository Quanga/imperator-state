...
DataService.prototype.createDataModel = function($happn) {
	...
	return (async () =>
		DataModel.create()
			.withMode(env.systemMode)
			.on("error", err => log.error("DataModel error:", err))
			.on("state", state => log.info("need to handle state", state))
...
DataService.prototype.clearDataModel = function($happn) {
	...
	return new Promise(resolve => {
		this.dataModel = DataModel.create().withMode(env.systemMode);
...
DataService.prototype.upsertNodeDataArr = function($happn, nodeArr) {
	...
	return (async () => {
		try {
			...
			const handle = await dataService.applyUpdates(context);
		...
		try {
			...
			const processUpdate = async unit => {
				...
				if (!upsertResult || !upsertResult.action) {
					return log.error("Return from dataModel is not in correct format");
				}
			...
			const tasks = nodeArr.map(unit => processUpdate(unit));
...
DataService.prototype.applyUpdates = function($happn, context) {
	const { dataService } = $happn.exchange;
	const { log } = $happn;
...
DataService.prototype.persistUnits = function($happn, updates) {
	...
	return (async () => {
		try {
			...
			await Promise.all(updates.map(update => insert(update)));
		} catch (error) {
