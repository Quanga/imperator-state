...
cacheService.prototype.writeDataIncrement = function($happn, options) {
	...
	return new Promise((resolve, reject) => {
		data.setSibling(options.path, options.data, (err, response) => {
			if (err) {
				logError("Write Data Increment Error", err);
				return reject(err);
			}

			this.state.items.push(response);
			console.log("items in queue", this.state.items.length);
			this.state.count++;
			emit("cache_set", name);
...
cacheService.prototype.writeData = function($happn, options) {
	return new Promise((resolve, reject) => {
		$happn.data.set(options.path, options.data, { noPublish: false }, function(
			e,
			response
		) {
