...
PacketService.prototype.extractData = function($happn, msgObj) {
	...
	return (async () => {
		try {
			const { packet, createdAt } = msgObj;

			const packetType = parseInt(packet.substr(6, 2), 16);
			if (this.__modes[env.mode].commands.indexOf(packetType) === -1) return null;
			...
			const parsedPacketArr = await parser.parse({ packet, createdAt });
			if (parsedPacketArr.length === 0) return null;
