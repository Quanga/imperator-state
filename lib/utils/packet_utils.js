...
PacketUtils.prototype.calculatePacketLength = function(command, serial, data) {
	const start = 4; // start field is always 4 hex chars
	const length = 2; // length field is always 2 hex chars
	const crc = 4; // crc field is always 4 hex chars
	const cmd = 2;
	const srl = 4;

	// each 2 hex chars = 1 byte (ie: 8 bits)
	let len = start + length + cmd + srl + data.length + crc;
	let byteLen = len / 2;
