...
class PacketUtils {
	...
	calculatePacketLength(command, serial, data) {
		const pattern = {
			start: 4, // start field is always 4 hex chars
			length: 2,// length field is always 2 hex chars
			crc: 4, // crc field is always 4 hex chars
			cmd: 2,
			srl: 4
		};

		// each 2 hex chars = 1 byte (ie: 8 bits)
		let len = pattern.start + pattern.length + pattern.cmd + pattern.srl + data.length + pattern.crc;
		let byteLen = len / 2;
