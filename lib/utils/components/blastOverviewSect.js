						...
						},
						{
							table: {
								...
								body: [
									...
									[
										{ text: " ", fontSize: 8 },
										{ text: dets.total, style: "tableContentCenter" },
										{ text: dets.connected, style: "tableContentCenter" },
										{
											text: dets.connected - dets.endConnected,
											style: "tableContentCenter"
										}
									]
								]
	...
	try {
		...
		for (let unitType of snapShotKeys) {
			...
			for (let unitKey of allUnits) {
				...
				result.fired += end[unitType][unitKey].units.detonatorStatusCount;
			}
		}
		return result;
	} catch (err) {
