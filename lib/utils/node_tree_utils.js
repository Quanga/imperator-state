...
NodeTreeUtils.prototype.extractUpdates = function($happn, context) {
	...
	const { nodeTreeUtils } = $happn.exchange;
	...
	let extractUpdatesAsync = async () => {
		...
		try {
			for (let pos = 0; pos < nodeArr.length; pos++) {
				...
				if (nodeToUpdate) {
					let mappedNode = await nodeTreeUtils.updateNode(
						nodeArr[pos],
						nodeToUpdate
					);
...
NodeTreeUtils.prototype.updateNode = function($happn, nextNode, prevNode) {
	const { dataMapper } = $happn.exchange;

	const updateAsync = async () => {
		let nextNodeClone = clone(nextNode);
		nextNodeClone.meta.status = "matched"; // UPDATED NODES

		let mappedNode = await dataMapper.mapUpdateNode(nextNode, prevNode);
		return mappedNode;
	};
