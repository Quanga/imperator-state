...
NodeTreeUtils.prototype.extractNewAndUpdatedNodes = function(
	...
	async function extractNewAndUpdatedNodesAsync() {
		var result = {
			updateNodes: [],
			newNodes: [],
			missingNodes: []
		...
		for (const node of nodeArr) {
			...
			try {
				...
				let updateNodeSerial = await self.setSerialByPosition(
					node,
					subTree,
					pos,
					nodeArr.length
			...
			if (updateNode != undefined) {
				...
				try {
					...
					//$happn.log.info("mapped return", mappedNode);
				...
				// NEW NODES
				node.happn_path = self.buildHappnPath(node, subTree);
...
NodeTreeUtils.prototype.findSubTree = function(
	...
	async function findSubTreeAsync() {
	...
	async function setSerialAsync() {
		...
		if (incomingNode.serial === null) {
			if (subTree[0].window_id == len - 1) {
				...
				if (ib651 != null) {
					console.log(">>>>>>>>>>>>>>>>>>>>>>", incomingNode.serial);
