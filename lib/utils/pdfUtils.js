...
module.exports = class pdfUtil {
	...
	marginInPoints(marginInMilimeter) {
			...
			content: [
				{
					...
					columns: [
								...
								table: {
									...
									body: [
										...
										[
											...
											{
												text: moment(reportObj.created).format("MMMM Do YYYY"),
												style: "standard"
											}
										],
										[
											...
											{
												text: moment(reportObj.created).format("h:mm:ss a"),
												style: "standard"
											}
										]
									]
						...
						{
							svg: `<svg width="214.33px" height="66.744px" enable-background="new 0 0 214.329 66.744" version="1.1" viewBox="0 0 214.329 66.744" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
						...
						//fit: [30, 30]
						//width: this.marginInPoints(20),
						//height: this.marginInPoints(20)
					]
				},
				{
					canvas: [
						{
							type: "line",
							x1: 0,
							y1: this.marginInPoints(10),
							x2: this.marginInPoints(180),
							y2: this.marginInPoints(10),
							lineWidth: 0.5,
							alignment: "center"
						}
					]
	...
	createReportContent(reportObj) {}

	async createPdf() {
		const pdfDoc = this.printer.createPdfKitDocument(this.docDefinition);
		await pdfDoc.pipe(fs.createWriteStream("document4.pdf"));
		pdfDoc.end();
	}
};
