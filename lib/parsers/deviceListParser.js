const { PacketModelList } = require("../models/PacketModel");
const PacketValidation = require("./CommandHandler/packetValidataion");
...
class DeviceListParser {
	...
	parse($happn, packetObject) {
		...
		console.log("PACKET OBJECT", packetObject);
		...
		try {
			...
			// const packetLength = packet.length - nonDataBytes;
			...
			if (this.packetTemplate.packetType === "mixed") {
				resultArr[0].data.childCount = bytePos - 1;
			}
