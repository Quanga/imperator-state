...
UidListParser.prototype.parse = function($happn, splitPacket) {
	...
	let buildNodeDataAsync = async () => {
		try {
			...
			console.log("CBB extract", JSON.stringify(parent, null, 2));

			// now add the child list
			let childUnits = splitPacket.data.match(/.{1,10}/g);

			for (let childData of childUnits) {
				let splitWindowData = utils.extractEddWindow(childData);
				//console.log("EDD extract", JSON.stringify(splitWindowData, null, 2));
				console.log(
					`from childUnits - ${childUnits} from childData - ${childData} split window -${JSON.stringify(
						splitWindowData,
						null,
						2
					)} `
