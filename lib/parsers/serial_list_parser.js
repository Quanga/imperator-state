...
SerialListParser.prototype.parse = function($happn, splitPacket, callback) {
	...
	try {
		...
		var parent = utils.createPacketResult(
			...
			splitPacket.crc,
			null,
			self.__constant.data.first_byte.parent_type
		);
		...
		splitPacket.data.match(/.{1,4}/g).forEach(function(item) {
			...
			var result = utils.createPacketResult(
				...
				splitPacket.crc,
				splitPacket.serial,
				self.__constant.data.remaining_bytes.parent_type,
				pos
			);
...
SerialListParser.prototype.buildNodeData = function(
	$happn,
	parsedPacketArr,
	callback
) {
