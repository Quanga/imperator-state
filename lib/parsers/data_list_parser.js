...
DataListParser.prototype.parse = function($happn, splitPacket, callback) {
	...
	try {
		console.log(":: data list parser ::");
		$happn.log.info(":: data list parser ::");

		var count = 1;

		if (splitPacket.data == "") return callback(null, resultArr);
		...
		splitPacket.data.match(/.{1,4}/g).forEach(function(item) {
			...
			if (count == 1) parentType = self.__constant.data.first_byte.parent_type;
			else parentType = self.__constant.data.remaining_bytes.parent_type;
			...
			else $happn.log.info("Invalid data detected and ignored!");
...
DataListParser.prototype.buildNodeData = function(
	$happn,
	parsedPacketArr,
	callback
) {
