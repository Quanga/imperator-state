class PacketModel {
	...
	extractRawData(data) {
		...
		case 5:
			if (this.typeId === 3) {
				result =
						data != null ? this._util.extractCbbRawData(this.rawData) : null;
				this.led_state =
						this.ledData != null ? this.ledData.toString() : null;
			} else {
				result =
						data != null ? this._util.extractEddRawData(this.rawData) : null;
			}
			break;
		...
		default:
			result =
					data != null ? this._util.extractPacketData(this.rawData) : null;
			break;
		}
