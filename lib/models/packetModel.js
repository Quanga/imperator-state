...
class PacketModel {
	constructor(packetObject) {
		...
		const requiredArgs = ["created", "packetTemplate", "packetSerial", "packet"];
		...
		const { pos, created, packetSerial, packetTemplate } = packetObject;

		if (!packetTemplate.payload) throw new Error("Incorrect packet template object");
	...
	extractEddRawData(rawData, extendWindowId) {
		...
		return {
			windowId: !extendWindowId
				...
				: this.util.reverseSerialBytes(parseInt(rawData.substr(0, 2 + offset), 16)),
			data: this.convertRaw(rawData.substr(2 + offset, 2), 0, 8, 8, 2),
			delay: this.util.reverseSerialBytes(parseInt(rawData.substr(4 + offset, 4), 16) || 0)
		};
	}
