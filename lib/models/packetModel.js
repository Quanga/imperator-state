...
class PacketModel {
	...
	validatePacket(packetObject) {
		...
		requiredArgs.forEach(arg => {
			if (!Object.prototype.hasOwnProperty.call(packetObject, arg)) errArr.push(arg);
		});
		...
		switch (this.template.command) {
			...
			case 5:
				if (this.data.typeId === 3) {
					...
					if (!process.env.MODE)
						throw new Error("mode is not set... check environment vairables are set");

					if (modes[process.env.MODE].extendedWindowId) extended = true;

					///console.log("MODES", modes[process.env.MODE].extendedWindowId);
