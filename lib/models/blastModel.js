...
const { blastStates } = require("../constants/stateConstants");
...
class BlastModel {
	constructor(snapshot, createdAt, firingDuration, reportingDuration) {
		...
		this.timer = this.setFiringTimer(createdAt, totalDuration);
		this.watchLists = snapShotProcessed.watchLists;
		this.expectedComplete = createdAt + totalDuration;

		this.setState(blastStates.BLAST_FIRING);
	}
	...
	aggregateUnits(units) {
		...
		unitKeys.forEach(unitKey => {
			...
			if (keySwitchStatus === 1 && unitsCount > 0) state = "ARMED";
			if (keySwitchStatus === 0 && unitsCount > 0) state = "EXCLUDED";
	 ...
	 */
	setFiringTimer(createdAt, duration) {
		return setTimeout(() => {
			this.setState(blastStates.BLAST_TIMER_COMPLETE);
	...
	addLog(logObj) {
		...
		if (
			...
			this.data.state !== blastStates.BLAST_DATA_COMPLETE
		) {
			this.setState(blastStates.BLAST_DATA_COMPLETE);
			this.closeBlast(createdAt);
		}
	}
	...
	handleUnitStateChange(logObj) {
		...
		switch (typeId) {
		case 0:
			if (logEvents.hasOwnProperty("fireButton") && logEvents.fireButton === 0) {
				this.data.firingComplete = createdAt;
				this.data.firingTime = this.data.firingComplete - this.data.createdAt;
				this.setState(blastStates.BLAST_FIRED);
			}
			// TODO need to get event for an DIRARMED TERMINATE
			if (
				logEvents.hasOwnProperty("keySwitchStatus") &&
					logEvents.keySwitchStatus === 0 &&
					this.state === blastStates.BLAST_FIRING
			) {
				this.data.firingComplete = createdAt;
				this.data.firingTime = 0;
				this.setState(blastStates.BLAST_TERMINATED);
				return false;
			}
			break;
		}
