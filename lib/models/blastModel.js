 ...
 */
class BlastModel {
	constructor(snapshot, createdAt, duration) {
		...
		if (!process.env.NODE_ENV === "test" && duration < 10000)
			throw new Error("Duration is too low for blast report- check env");
		...
		this.expectedComplete = createdAt + duration;
	 ...
	 */
	addLog(logObj) {
		if (logObj.createdAt > this.expectedComplete) {
			return this.closeBlast(logObj.createdAt, true);
		}

		this.data.logs.push(logObj);
		if (logObj.logType === "UNIT_UPDATE") {
			//check to the fireButton off to complete firing part
			if (
				logObj.typeId === 0 &&
				logObj.hasOwnProperty("events") &&
				logObj.events[0].hasOwnProperty("fireButton") &&
				logObj.events[0].fireButton === 0
			) {
				this.data.firingComplete = logObj.createdAt;
		...
		if (logObj.logType === "DET_UPDATE") {
			if (logObj.typeId === 4) {
				//check to see if it is in the list
				if (this.blastWatch.watchDets.hasOwnProperty(logObj.parentSerial.toString())) {
					const checkDetUnit = this.blastWatch.watchDets[logObj.parentSerial.toString()].findIndex(
						x => x === logObj.windowId.toString()
					);

					if (checkDetUnit !== -1) {
						this.blastWatch.watchDets[logObj.parentSerial].splice(checkDetUnit, 1);
						if (this.blastWatch.watchDets[logObj.parentSerial].length === 0) {
							delete this.blastWatch.watchDets[logObj.parentSerial];
						}
					}
				}
			}
		...
		if (
			...
			this.closeBlast(logObj.createdAt);
		}
	}
