...
class BlastModel {
	...
	addLog(logObj) {
		...
		if (logObj.value[0].modifiedAt > this.expectedComplete) {
			return this.closeBlast(logObj.value[0].modifiedAt, true);
		}
		...
		for (const log of logObj.value) {
			...
			if (
				log.typeId === 0 &&
				log.events[0].hasOwnProperty("fireButton") &&
				log.events[0].fireButton === 0
			) {
				this.data.firingComplete = log.createdAt;
			...
			if (
				...
				this.closeBlast(log.modifiedAt);
			}
		}
	}
