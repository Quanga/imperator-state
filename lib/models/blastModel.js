...
class BlastModel extends EventEmitter {
	...
	withFSM(fsm) {
		const activities = {
			actions: {
				update: assign((context, event) => {
					return { createdAt: event.createdAt, message: "Count changed" };
				}),
			},
			activities: {
				firingCountDown: () => {
					...
					firingTimer
						.on("tick", ms => {
							console.log({ timer: "firing", duration: Math.round(ms / 1000) * 1000 });
				...
				reportCountDown: () => {
					...
					return () => {
						console.log("stopping report timer");
						firingTimer.stop();
						this.emit("timer", { timer: "report", duration: 0 });
					};
				},
		...
		this.state.fsm = Machine(fsm, activities);
		this.state.fsmService = interpret(this.state.fsm).onTransition(state => {
			if (state.changed) {
				this.state.currentState = state.value;
				console.log("EMITTING BLAST", { createdAt: state.context.createdAt, state: state.value });
				this.emit("state", { createdAt: state.context.createdAt, state: state.value });
			}
		});
	...
	/* { serial: 8,
  typeId: 0,
  createdAt: 1575010208376,
  state: { operation: 'firing_complete', faults: 'none' } }
  */
	toggleState(obj) {
		console.log(obj);
		let trigger;
		...
		if (trigger) this.state.fsmService.send(trigger, { created: obj.createdAt });
	}
