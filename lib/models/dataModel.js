...
class DataModel extends EventEmitter {
	...
	async updateUnit(prevState, nextState, diffs, forceFlag) {
		...
		try {
			...
			case 0:
				{
					nextState.data = await this.applyUpdate(prevState.data, diffs, created, forceFlag);
	 ...
	 * @returns {Promise<unitObj>} return found unit or null
	 */
	async applyUpdate(state, diff, modified, forceFlag) {
		try {
			if (diff) {
			...
			if (forceFlag === true) state.modified = modified;
			//console.log(`state - ${state}  diff -${diff}  modified -${modified}  force-${forceFlag}`);
