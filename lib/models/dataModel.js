...
class DataModel {
			...
			case 3:
				{
					...
					await this.updateBoosterCounts(unit, {
						keySwitchStatus,
						communicationStatus
					});
				}
	...
	upsertUnit(nextState) {
		const updateUnitAsync = async () => {
			try {
				...
				if (!diffs) return { action: "NONE", value: nextState };
				...
				case 3:
					{
						await this.updateBoosterCounts(nextState, diffs);
						nextState.data = await this.applyUpdate(prevState.data, diffs);
	...
	clearEdds(serial) {
		const clearAsync = async () => {
			...
			const dets = clone(unit.children);

			return { action: "CLEAR", value: dets };
		};
		return clearAsync();
	}
}
