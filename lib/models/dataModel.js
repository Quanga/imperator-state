...
class DataModel extends EventEmitter {
	...
	commitUpsert(setObj, ignoreFSM) {
		try {
			...
			switch (action) {
				...
				case "UPDATE":
					this.updateUnit(prevState, nextState, index, unitKey, ignoreFSM);
					break;
	 ...
	 */
	updateUnit(prevState, nextState, indexType, unitKey, ignoreFSM = false) {
		try {
	...
	async disarmBoosters() {
		try {
			...
			const units = Object.keys(upserts).map((unit) => this.commitUpsert({ ...upserts[unit] }, true));
	...
	async disconnectChildren(context) {
		try {
			if (this.units["4"][context.serial]) {
	...
	deleteChildren(parent, indexs, childTypeId = 4) {
		try {
			...
			const result = Object.keys(indexs).map((unit) => ({
				...this.units[childTypeId][pSerial][unit],
			}));
