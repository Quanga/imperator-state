...
class DataModel extends EventEmitter {
	...
	async disarmBoosters(context) {
		const children = clone(this.units["3"]);

		const offChild = Object.keys(children).forEach(childKey => {
			children[childKey].fsm.fsmService.send("COMM_LOST");
		});

		const upserts = await Promise.all(
			offChild.map(unit => {
				return this.stageUpsert(unit, true);
			}),
		);

		const units = [];
		for (const unit of upserts) {
			const result = await this.commitUpsert({ ...unit });
			units.push({ ...result });
		}
