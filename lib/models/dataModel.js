			...
			case 3:
				{
					...
					this.units[serial].on("COMMS_LOST", () => {
						let clonedUnit = clone(unit);
						clonedUnit.data.communicationStatus = 0;
						clonedUnit.data.modified = Date.now();
						console.log("DATA MODEL SEES COMMS LOST FOR", clonedUnit.data.serial);
			...
			case 3:
				{
					...
					if (nextState.data.communicationStatus) {
						nextState.setLastCommunication(nextState.data.modified);
					}
