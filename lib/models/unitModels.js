...
class UnitModel extends EventEmitter {
	...
	get children() {
		return this.meta._children;
	}
	set children(value) {
		if (this.meta._children !== value) {
			this.meta.children = value;
			this.emit("children", this.meta.children);
		}
	}

	emitter() {
		console.log(`emitting for ${this.data.serial} type - ${this.data.type_id}`);
		this.emit(
			"started",
...
class CBoosterModel extends UnitModel {
	...
	checkComs() {
		...
		this.meta.communicationCountDown = setTimeout(() => {
			...
			this.data.communication_status = 0;
