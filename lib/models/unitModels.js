...
class ControlUnitModel extends UnitModel {
	constructor(serial) {
		...
		this.data.typeId = unitTypes.CONTROL_UNIT;
		//status
...
class BoosterModel extends UnitModel {
	constructor(serial, parentSerial) {
		...
		this.data.typeId = unitTypes.BOOSTER_T1;
...
class CBoosterModel extends UnitModel {
	constructor(serial, ledState, childCount) {
		...
		this.data.typeId = unitTypes.BOOSTER_T2;
		this.data.parentType = unitTypes.CONTROL_UNIT;
	...
	updatePseudoTime(packetTime) {
		if (packetTime > this.func.expectedTimeoutTime && this.func.communicationCountDown) {
			//clearTimeout(this.meta.communicationCountDown);
			this.func.expectedTimeoutTime = packetTime + parseInt(process.env.COMMUNICATION_CHECK_INTERVAL);
	...
	setLastCommunication(started) {
		...
		// 	clearTimeout(this.meta.communicationCountDown);
		// 	this.meta.expectedTimeoutTime = started +
		//parseInt(process.env.COMMUNICATION_CHECK_INTERVAL);
		// }
	...
	emitTimeout() {
		...
		this.func.communicationCountDown = undefined;
	}
}
...
class EDDModel extends UnitModel {
	constructor(serial, parentSerial, windowId, delay) {
		...
		this.data.typeId = unitTypes.EDD;
		this.data.parentType = unitTypes.BOOSTER_T2;
...
class CFCModel extends UnitModel {
	constructor(serial, qos, qosFiring, firmware) {
		super(serial);
		this.data.typeId = unitTypes.CFC;
		this.data.qos = qos;
