...
class CBoosterModel extends UnitModel {
	constructor(serial, parentSerial) {
		...
		this._lastCommunication = new Date();
	}

	set lastCommunication(value) {
		this.communication_status = 1;
		this._lastCommunication = value;
		this.checkComs();
	}

	checkComs() {
		console.log(parseInt(process.env.COMMUNICATION_CHECK_INTERVAL));
		if (this.communicationCountDown != undefined) {
			clearTimeout(this.communicationCountDown);
			console.log("CLEARING TIMER");
		}
		console.log("STARTING TIMER");
		this.communicationCountDown = setTimeout(() => {
			console.log("TIMER DONE");
			this.communication_status = 0;
			delete this.communicationCountDown;
